...
def book_list(request):
    ...
    print('Book List from Session:', book_list)
    
    # Filter books based on whether they are finished reading or not
    books = Book.objects.filter(pk__in=book_list)[:5]
    print('Books:', books)
...
def book_new(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('book_list')  # Redirect to book_list instead of book_detail
        else:
...
def write_review(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            review.book = book
            review.save()
            return render(request, 'books/review_success.html', {'book': book, 'review': review})
    else:
        form = ReviewForm()
