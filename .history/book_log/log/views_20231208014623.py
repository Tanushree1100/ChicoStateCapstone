...
def index(request):
    if request.method == 'POST':
        ...
        for book_data in results:
            ...
            Book.objects.create(title=title, authors=', '.join(authors))

        return render(request, 'log/landing_page.html', {'results': results, 'query': query})

    # Display the saved results on the landing page
    saved_results = Book.objects.all()
    return render(request, 'log/landing_page.html', {'results': saved_results, 'query': ''})